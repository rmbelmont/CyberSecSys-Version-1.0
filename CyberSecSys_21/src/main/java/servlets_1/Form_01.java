...
public class Form_01 extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
      ...
      try {
         ...
         if (view != null) {
            if (view.isCarregado()) {
                resultado.addAll(view.getApproach());
            } else {
                resultado.add("Aviso: CyberSecView está carregando em segundo plano. Tente novamente em alguns instantes.");
                try {
                    resultado.addAll(view.getApproach());
                } catch (Exception e) {
                    resultado.add("Dados ainda não disponíveis. Aguarde o carregamento completo.");
                }
            }
         } else {
            resultado.add("Erro: CyberSecView não foi carregada no contexto da aplicação.");
            try {
                CyberSecView view2 = CyberSecView.getInstance();
                req.getServletContext().setAttribute("cyberSecView", view2);
                resultado.add("Nova instância de CyberSecView criada. Recarregue a página.");
            } catch (Exception e) {
                resultado.add("Falha ao criar CyberSecView: " + e.getMessage());
            }
         }
      ...
      printWriter.print("      background: url('data:image/svg+xml,<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 100 100\"><defs><pattern id=\"grid\" width=\"10\" height=\"10\" patternUnits=\"userSpaceOnUse\"><path d=\"M 10 0 L 0 0 0 10\" fill=\"none\" stroke=\"rgba(255,255,255,0.1)\" stroke-width=\"0.5\"/></pattern></defs><rect width=\"100\" height=\"100\" fill=\"url(%23grid)\" /></svg>');");
      ...
      printWriter.print("      margin-bottom: 2rem;");
      printWriter.print("    }");
      printWriter.print("    .warning-message {");
      printWriter.print("      background: #fff3cd;");
      printWriter.print("      border-left: 4px solid #ffc107;");
      printWriter.print("      color: #856404;");
      printWriter.print("      padding: 1rem 1.5rem;");
      printWriter.print("      border-radius: 8px;");
      printWriter.print("      margin-bottom: 2rem;");
      printWriter.print("    }");
      ...
      printWriter.print("      <h2 class='content-title'>Cybersecurity Performance Approaches</h2>");
      
      if (!lista.isEmpty()) {
         String primeiroItem = lista.get(0);
         if (primeiroItem.startsWith("Erro")) {
            printWriter.print("      <div class='error-message'>");
            printWriter.print("        <i class='fas fa-exclamation-triangle me-2'></i>");
            printWriter.print("        " + primeiroItem);
            printWriter.print("      </div>");
         } else if (primeiroItem.startsWith("Aviso")) {
            printWriter.print("      <div class='warning-message'>");
      ...
      if (lista.isEmpty() || lista.get(0).startsWith("Erro") || lista.get(0).startsWith("Aviso")) {
         if (lista.isEmpty()) {
         ...
         for(int i = 0; i < lista.size(); ++i) {
            ...
            printWriter.print("          <strong>" + lista.get(i) + "</strong>");
